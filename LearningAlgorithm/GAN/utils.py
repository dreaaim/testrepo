...
def merge(images, size):
	...
	if(images.shape[3] in (3,4)):
		...
		for idx, image in enumerate(images):
			i = idx % size[1]
			j = idx // size[1]
			img[j * h: j * h + h, i * w + w] = image[:,:,0]
		return img
	else:
		raise VauleError('in merge(images,size) images parameter '
...
def visualize(sess, dcgan, config, option, sample_dir='samples'):
	...
	if option == 0:
		...
		for idx in xrange(dcgan.z_dim):
			...
			samples = sess.run(dcgan.sampler, feed_dict={dcgan.z: z_sample})
			save_images(samples, [image_frame_dim, image_frame_dim], os.path.join(sample_dir, 'text_arange_%s.png' % (idx)))
	elif option ==2:
		values = np.arange(0, 1, 1./config.batch_size)
